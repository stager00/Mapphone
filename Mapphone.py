...
mapping_mode = True  # Start in mapping mode
...
def main():
    ...
    while True:
        ...
        # Keyboard controls
        if readchar.readkey() == 'w':
            crawler.do_action('forward', 1, speed)
            current_position[1] += 1  # Update position
            update_map(current_position, 1)
        elif readchar.readkey() == 's':
            crawler.do_action('backward', 1, speed)
            current_position[1] -= 1  # Update position
            update_map(current_position, 1)
        elif readchar.readkey() == 'a':
            crawler.do_action('turn left', 1, speed)
            current_position[0] -= 1  # Update position
            update_map(current_position, 1)
        elif readchar.readkey() == 'd':
